...
(async () => {
  ...
  const changelogContent = changelogs.map(changelog => `# ${changelog.projectName}
...
${changelog.diff ?? ""}`).join("\n\n")
  ...
  const englishReleaseNote = await gemini(model, `Summarize the following individual release notes to a human-friendly, marketing oriented release note in Markdown format using the following sections: short intro, new features & enhancements and bug fixes.\n\n${changelogContent}`);
  ...
  for (let [language, changelog] of Object.entries(humanFriendlyReleaseNotes)) {
    await createOrUpdateFile({
      ...baseParam,
      branch: branchRefs.RELEASE_NOTE_BRANCH,
...
async function getChangelogs(octokit: ReturnType<typeof getOctokit>, config: ConfigSchema) {
  ...
  await Promise.all(Object.entries(config).map(async ([repositoryName, c]) => {
