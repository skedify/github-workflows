...
${changelog.diff ?? ""}`,
  ...
  const prompt = `Summarize the following individual release notes to a human-friendly, marketing oriented release note in Markdown format using the following sections: short intro, new features & enhancements and bug fixes.\n\n${changelogContent}`;
  core.info(prompt);
  core.info(prompt.length.toString());

  const englishReleaseNote = await gemini(model, prompt);
  core.info(englishReleaseNote);
